...
from app.models.models import InternalAlert, MercadoPagoSettings, PosLocation, PosSale, Product, StorefrontConfig, Tenant, User
...
def evaluate_tenant_operational_alerts(db: Session, tenant: Tenant) -> list[_AlertCandidate]:
    ...
    escalation = "Base de escalamiento lista: correo, Telegram y bot interno."
    ...
    metrics = [
        ("limit_brands", "Límite de marcas", brands_used, brands_limit),
        ("limit_users", "Límite de usuarios", users_used, users_limit),
        ("limit_ai_agents", "Límite de agentes IA", ai_agents_used, ai_agents_limit),
        ("limit_products", "Límite de productos", products_used, products_limit),
        ("limit_branches", "Límite de sucursales", branches_used, branches_limit),
    ]
    for code, label, used, limit in metrics:
        sev = _usage_severity(used, limit)
        if not sev:
            continue
        status_text = "alcanzado" if used >= limit else "cerca de límite"
        candidates.append(
            _AlertCandidate(
                code=code,
                title=f"{label}: {status_text}",
                message=f"{label}: {used}/{limit}. {escalation}",
                severity=sev,
            )
        )
    ...
    if credit_snapshot.key_state == "bloqueada":
        candidates.append(
            _AlertCandidate(
                ...
                message=f"La marca agotó créditos IA asignados ({credit_snapshot.consumed_tokens}/{credit_snapshot.assigned_tokens}). {escalation}",
                severity="high",
            )
        )
        ...
        if pct <= 0 or not scope:
            candidates.append(
                _AlertCandidate(
                    ...
                    title="Comisión activa sin configuración suficiente",
                    message="La comisión está activa pero falta porcentaje válido o scope operativo. Revisar configuración comercial.",
                    severity="high",
                )
            )
    ...
    if expansion_enabled and not country_code:
        candidates.append(
            _AlertCandidate(
                ...
                title="Expansión activa sin país configurado",
                message="La marca tiene expansión habilitada sin país activo definido.",
                severity="warning",
            )
        )
        ...
        if not str(currency_settings.base_currency or "").strip():
            candidates.append(
                _AlertCandidate(
                    ...
                    title="Expansión activa sin moneda base",
                    message="La marca tiene expansión habilitada sin configuración de moneda base operativa.",
                    severity="warning",
                )
            )
    ...
    if feature_logistics and active_branches <= 0:
        candidates.append(
            _AlertCandidate(
                ...
                title="Logística contratada sin configuración",
                message="El módulo de logística está habilitado pero no hay sucursales activas para operar.",
                severity="warning",
            )
        )
        ...
        if active_users <= 0:
            candidates.append(
                _AlertCandidate(
                    ...
                    title="Jornada laboral contratada sin configuración",
                    message="El módulo de jornada laboral está habilitado pero no hay usuarios activos para operarlo.",
                    severity="warning",
                )
            )
    ...
    if feature_nfc and not bool(channel_settings.get("nfc_enabled", False)):
        candidates.append(
            _AlertCandidate(
                ...
                title="NFC contratado sin configuración",
                message="El módulo NFC está habilitado pero no está activado en configuración de canales.",
                severity="warning",
            )
        )
        ...
        if not mp_settings or not bool(mp_settings.mercadopago_enabled):
            candidates.append(
                _AlertCandidate(
                    ...
                    message="NFC está habilitado sin integración activa de pagos POS (Mercado Pago).",
                    severity="info",
                )
            )
